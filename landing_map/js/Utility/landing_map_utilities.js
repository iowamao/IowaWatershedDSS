var watershed_search = {
    "NHD_search_point": function(lat, lng) {
        ...
        ws_pin.addLayer(L.marker([lat, lng], {
            icon: pushpin()
        }));
        ...
        $.ajax({
            ...
            //url: "php/NHD_trace/elite_search/point_fast_search.php?lat=" + lat + "&lng=" + lng,
			url: "php/NHD_trace/elite_search/point_aggregation.php?lat=" + lat + "&lng=" + lng,
    ...
    "NHD_search_huc": function(huc, feature) {
        ...
        var result_display = L.geoJson(null, {
            style: layer_style["Huc_poi_style"],
        ...
        leaflet_layer_setstyle([HUC8_global, City_global, County_global], layer_style.project_default_style);
    },
...
function draw_ifis_boundary(json_data, fit) {
    ...
    var result_display = L.geoJson(json_data, {
        style: layer_style["Huc_ws_trace_style"],
        onEachFeature: function(feature, layer) {
...
function lauch_state_app(show) {
    if (show == "show") {
        //var style="style='position:absolute; top:75px; right:10px; width:240px;background:#336600 linear-gradient(to bottom, #33cc33 5%, #009900 100%) repeat scroll 0 0; border:1px solid #006600;box-shadow:0 1px 0 0 #003300 inset'";
        var style = "style='position:absolute; top:125px; right:9px; width:126px; zIndex:120;";
        var state_button = "<button id='the_statelaunchButton' class='btn btn-success btn-lg' " + style + "><b><span style='font-size:22px'>Launch <br>IoWaDSS</span></b></button>"
        $("body").append(state_button);
...
function show_launch_button(show_button) {
    //var style="style='position:absolute; top:75px; right:10px; width:240px;background:#336600 linear-gradient(to bottom, #33cc33 5%, #009900 100%) repeat scroll 0 0; border:1px solid #006600;box-shadow:0 1px 0 0 #003300 inset'";
    var style = "style='position:absolute; top:125px; right:9px; width:126px; zIndex:120;";
    var button = "<button id='the_launchButton' class='btn btn-success btn-lg' " + style + "><b> <span style='font-size:22px'>Launch <br>IoWaDSS</span></b></button>"
...
function show_map_legend(show_map_legend) {
    ...
    var style = "style='position: absolute; bottom: 40px; left: 1%; border-style: solid; padding: 5px 10px; background-color: rgba(255, 255, 255, 0.5); border-color: rgba(255, 255, 255, 0.9);'";
    var control = {
        "show": function() {
    ...
    var control = {
        "show": function() {
            if ($("#map_legend2").length <= 0) {
                ...
                $("#map").append("<div id='map_legend2' style='position:absolute; top:2%; left:1%;width:700px;height:80px; font-size:15px; '>" + html + "</div>");
...
function third_level_ws_boundary(feature, layer) {
    ...
    setTimeout(function() {
        ...
        $("#the_launchButton").on("click", function() {
            ...
            laoding_spinner("show");
            var map_fit_bound = [
                [window["third_level_boundary"]["_southWest"]["lat"], window["third_level_boundary"]["_southWest"]["lng"]],
                [window["third_level_boundary"]["_northEast"]["lat"], window["third_level_boundary"]["_northEast"]["lng"]]
            ];
            ...
            polygon_add_outer_boundary(parsed_feature, outer_boundary_mask, function() {
                ...
                //
                $.post("php/watershed.php", {
                        feature_geom: JSON.stringify(parsed_feature),
                        poi_geom: JSON.stringify(parsed_poi_feature)
                    })
                    .done(function(data) {
...
function show_launch_button_project(show_button) {
    //var style="style='position:absolute; top:75px; right:10px; width:240px;background:#336600 linear-gradient(to bottom, #33cc33 5%, #009900 100%) repeat scroll 0 0; border:1px solid #006600;box-shadow:0 1px 0 0 #003300 inset'";
    var style = "style='position:absolute; top:125px; right:9px; width:126px; zIndex:120;";
    var button = "<button id='project_launchButton' class='btn btn-success btn-lg' " + style + "><b> <span style='font-size:22px'>Launch <br>IoWaDSS</span></b></button>"

    if (show_button == "show") {
        if ($("#project_launchButton").length == 0) {
            $("body").append(button);
        }
    }
    if (show_button == "hide") {
        $("#project_launchButton").remove();
    }
    
}

function third_level_pj_boundary(feature,layer,style_object) {

    //console.log("new pj json");
    //console.log(feature);
	var original_style=style_object["original"];	
	var poi_style=style_object["poi"];
	
    layer.on('click', function(e) {
		
		var lv3_bound = JSON.stringify(find_bound_box(feature.geometry.coordinates[0]));
		//show_launch_button_project("hide");	
		show_launch_button_project("show");
		show_map_legend("show");
		
		
		$(document).on("click","#project_launchButton",function() {
			btn_event(feature, layer);	
		});
        ...
        function btn_event(feature, layer) {
    ...
    function find_bound_box(coordinate_group) {
        ...
        return [
            [lng[0], lat[lng.length - 1]],
            [lng[lat.length - 1], lat[0]]
        ]
    }
