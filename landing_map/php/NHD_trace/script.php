...
$hash_file_huc12 = 'hash_huc12.json'; //this is a hash table that convert comid(catchment) to HUC12 
...
$query_join = "SELECT ST_AsGeoJSON(1,ST_UNION(geom),4,0) FROM (SELECT geom FROM public.catchment where featureid in (".$catch_res_text.") UNION select geom from public.huc12 where huc_12 in (".$huc_12_res_text.") UNION select geom from public.huc10 where huc_10 in (".$huc_10_res_text.") UNION select geom from public.huc8 where huc_8 in (".$huc_8_res_text.")) as t";
...
$ws_boundary->huc->huc_full_cid=$visited;   //attributes of the containing huc12
