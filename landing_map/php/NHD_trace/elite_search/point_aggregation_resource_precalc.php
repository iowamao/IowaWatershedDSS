...
$final_result="GRIDCODE,C_D,nodata,B_D,C,B,A,D,A_D,total";
...
file_put_contents("output2/culvert_ssurgo_hydro_soil.txt", $final_result);
...
*/
$query_join = "SELECT SUM(land.ssurgo_hydrosoil.C_D),SUM(land.ssurgo_hydrosoil.nodata),SUM(land.ssurgo_hydrosoil.B_D),SUM(land.ssurgo_hydrosoil.C),SUM(land.ssurgo_hydrosoil.B),SUM(land.ssurgo_hydrosoil.A),SUM(land.ssurgo_hydrosoil.D),SUM(land.ssurgo_hydrosoil.A_D),SUM(land.ssurgo_hydrosoil.total) FROM land.ssurgo_hydrosoil INNER JOIN nhd_structure.river_master ON land.ssurgo_hydrosoil.gridcode=nhd_structure.river_master.gridcode WHERE nhd_structure.river_master.featureid in (".$catch_res_text.") OR nhd_structure.river_master.huc_12 in (".$huc_12_res_text.") OR nhd_structure.river_master.huc_10 in (".$huc_10_res_text.") OR nhd_structure.river_master.huc_8 in (".$huc_8_res_text.")";
...
while ($row = pg_fetch_row($res2)) {
	...
	*/
	...
	$tempres=$output_name.",".$final_percentage[0].",".$final_percentage[1].",".$final_percentage[2].",".$final_percentage[3].",".$final_percentage[4].",".$final_percentage[5].",".$final_percentage[6].",".$final_percentage[7].",".$final_percentage[8];
