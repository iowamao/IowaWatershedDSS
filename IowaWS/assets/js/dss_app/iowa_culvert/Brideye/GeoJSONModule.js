...
var GeoJSONModule = function () {
    ...
    * Private Methods
    ******************/
    function parseGeoJSON(json,angle) {
	

        if (_callback != null) {
            var result = parseGeoJSONItem(json,angle);
            var bounds;
  ...
  /*          if (_allCoords != null && _allCoords.length > 0) {
    ...
    function parseGeoJSONItem(i,angle) {
        var shape = null;
		//console.log(angle);

        switch (i.type) {
            case "FeatureCollection":
                shape = parseFeatureCollection(i,angle);
                break;
            case "Feature":
                shape = parseFeature(i,angle);
				//console.log("geojson item feature");
                break;
            case "Point":
    ...
    function parseGeometryCollection(gc) {
        ...
        $(gc.geometries).each(function (i, v) {
            var entity = parseFeature({ geometry: v },0);
    ...
    function parseFeature(fe,angle) {
	//	console.log("pf");
	//	console.log(angle);
        var entity;
        var i = fe.geometry;
		//alert(fe.properties.name10);
        switch (i.type) {
            case "Point":
                var point = parsePoint(i);
				var origian_direction=fe.properties.dirmean;
				var newdirection=0;

if(fe.properties.dirmean!=0){

				if(origian_direction+angle>=0&&fe.properties.dirmean+angle<=360){
				newdirection=origian_direction+angle;
				}
				if(origian_direction+angle>360){
					if(angle>=360){

					newdirection=origian_direction+angle-parseInt(angle/360)*360;
					}else{
					newdirection=origian_direction+angle-360;
						}
				}
				if(origian_direction+angle<0){
					if(angle<=0){
					newdirection=origian_direction+angle+parseInt(-angle/360)*360;
					}else{
				newdirection=origian_direction+angle+360;
                ...
                entity = dirction_icon(point,newdirection);
				...
				Microsoft.Maps.Events.addHandler(entity, 'click',function(){
			 var center = new Microsoft.Maps.Location(i.coordinates[1], i.coordinates[0]);
			 

      //   setmepview(center);
	//	zoommepview();			
					
				//	alert(i.coordinates[1]+" "+i.coordinates[0]);
    ...
    function parseFeatureCollection(fc,angle) {
        var entityCollection = new Microsoft.Maps.EntityCollection();
        $(fc.features).each(function (i, v) {
            entityCollection.push(parseFeature(v,angle));
        });
        return entityCollection;
    }
    ...
    /* methods for creating Bing Maps entities including passed in options */
	 ...
	 function dirction_icon(point,d){	
...
//console.log(d);
	...
	return new Microsoft.Maps.Pushpin(point,{icon:'data/dss/IDOT/state_data/direction_lable_s/A'+pic+'.png', height:25, width:25, anchor:new 
Microsoft.Maps.Point(0,0), draggable: false});
    ...
    function createPushpin(point,d) {
		
		
		 return d > 0.8  ? new Microsoft.Maps.Pushpin(point,{icon:"graphic/L80.png", height:20, width:20, anchor:new 
Microsoft.Maps.Point(0,0), draggable: true})   :           
           d > 0.6  ? new Microsoft.Maps.Pushpin(point,{icon:"graphic/L60.png", height:20, width:20, anchor:new 
Microsoft.Maps.Point(0,0), draggable: true}) :
           d > 0.4   ? new Microsoft.Maps.Pushpin(point,{icon:"graphic/L40.png", height:20, width:20, anchor:new 
Microsoft.Maps.Point(0,0), draggable: true}) :
           d > 0.2   ? new Microsoft.Maps.Pushpin(point,{icon:"graphic/L20.png", height:20, width:20, anchor:new 
Microsoft.Maps.Point(0,0), draggable: true}) :
           d > 0   ? new Microsoft.Maps.Pushpin(point,{icon:"graphic/L0.png", height:20, width:20, anchor:new 
Microsoft.Maps.Point(0,0), draggable: true}) :
                      new Microsoft.Maps.Pushpin(point,{icon:"graphic/LN.png", height:20, width:20, anchor:new 
Microsoft.Maps.Point(0,0), draggable: true});
...
$(".NavBar_rotateLeft").click(function(e) {
		
//$(document).on('click','.NavBar_rotateLeft',function(){
	console.log("as");
clearmap();
changeangle+=90;
...
$( document ).ready(function() {
	...
	$(".NavBar_rotateRight").click(function(e) {
//$(document).on('click','.NavBar_rotateRight',function(){
//console.log("as");
clearmap();
changeangle-=90;
...
//$("#close_culvert_properties").click(function(e) {
            parseGeoJSON(json,0);
    	    maptool.instance.setView({zoom:10});
changeangle=0;
    });		
